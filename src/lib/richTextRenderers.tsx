...
/**
 * Custom Rich Text Renderers - Bridging Uniform Rich Text → Styled React
 *
 * Uniform stores rich text as a tree of nodes (paragraphs, headings, lists,
 * assets, quotes, etc.). By default `UniformRichText` renders these with
 * unstyled HTML tags. This file overrides that default for the node types
 * we care about so the output matches the rest of the site's design system.
 *
 * How it's wired up:
 *   <UniformRichText
 *     parameterId="..."
 ...
 *   />
 *
 * `resolveRichTextRenderer` is called once per node in the tree. For each
 * node it returns a React component to use, or `undefined` to fall back to
 * Uniform's default rendering (which is what we want for nodes we don't
 * customize, like inline `text` runs, `link`, `bold`, `italic`, etc.).
 *
 * What's customized here:
 *   - heading        → tag-aware sizing (h1–h6) with shared typography
 *   - paragraph      → adds spacing, plus detects "---" / "***" / "___"
 *                      sequences and renders them as a decorative divider
 *   - list / listitem→ Tailwind-styled ordered & unordered lists
 *   - asset (image)  → Next.js <Image> in a 4:3 rounded frame, with an
 *                      optional caption pulled from the asset's `title`
 *   - quote          → blockquote with left accent bar and tinted background
 *
 * To customize another node type:
 *   1. Add a new `RichTextRendererComponent` below.
 *   2. Add a matching `if (node.type === '...')` branch in
 *      `resolveRichTextRenderer` at the bottom of the file.
 */
