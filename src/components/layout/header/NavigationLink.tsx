 ...
 *
 * Header navigation item for the `navigationLinks` slot. Renders as a
 * shadcn ghost `Button` wrapping a `next/link` (or native `<a>` for
 * `email`/`tel` links).
 *
 * Link parameter (`linkUrl`):
 * - `projectMapNode` → resolves `linkData.path` via Uniform project map.
 * - `url` → internal or external path via `next/link`.
 * - `email` / `tel` → native `<a href={linkData.path}>`.
 * - Missing/unknown type → falls back to `href="#"`.
 *
 * Mobile vs desktop styling:
 * - Pass `isMobile={true}` explicitly, or rely on auto-detection: the
 *   component checks for a `.mobile-navigation-context` ancestor (set by
 *   `Header`'s mobile nav wrapper).
 * - Mobile: full-width left-aligned button with larger padding.
 * - Desktop: compact inline button.
 *
 * On click in mobile context, calls `useMobileMenu().close()` so the
 * drawer collapses after navigation.
 */
