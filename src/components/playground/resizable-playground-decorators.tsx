...
const defaultSizes: Record<ComponentType, keyof typeof sizes> = {
  ...
  faqSection: "Full Width",    // FAQ section spans full width
  faqItem: "MD",               // Single FAQ row at medium width
...
/**
 * ResizablePlaygroundDecorator
 *
 * Uniform playground decorator that constrains the preview to a selectable
 * max-width, simulating Tailwind breakpoints without leaving the editor.
 *
 * Behavior:
 * - Reads `data.type` from the component being previewed and picks a
 *   sensible default width from `defaultSizes` (e.g. `cta` → 640px,
 *   `cardGrid` → full width).
 * - Renders breakpoint buttons (320px–1536px + full width) below the
 *   preview; selected width is applied as `maxWidth` on the container.
 * - Buttons carry `IS_RENDERED_BY_UNIFORM_ATTRIBUTE` so Uniform treats
 *   them as editor chrome, not preview content.
 * - Re-syncs default size when the previewed component type changes.
 */
