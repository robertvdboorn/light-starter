 ...
 *
 * Renders a single image from a Uniform asset parameter, with an editable
 * title above and an italic caption below. A good reference for how to
 * pipe a Uniform asset through `imageFrom(...).transform(...)` into
 * `next/image`.
 *
 * What this component does:
 * - Reads the first asset from the `image` parameter (Uniform assets come
 *   as arrays even when only one is selected).
 * - Transforms it to 800×600 with `fit: "cover"`, honoring the asset's
 *   `focalPoint` field so cropping respects the author's intent.
 * - Wraps the result in a 4:3 `aspect-ratio` box with `next/image` `fill`
 *   so it stays responsive without layout shift.
 * - Falls back to a gray 4:3 placeholder with an icon and hint text when
 *   no asset is selected.
 * - Pulls alt text from the asset's `description` field, then `title`,
 *   then a generic fallback — so authors don't need to think about a11y.
 */
