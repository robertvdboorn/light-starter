 ...
 *
 * The top section of an article detail page: category badge, title,
 * excerpt, a metadata row (date / reading time / author), and an optional
 * featured image.
 *
 * Composition:
 * - Title:    UniformText, rendered as `<h1>` (this is the page's primary
 *             heading, so no other component on the page should render h1).
 * - Excerpt:  UniformRichText, so authors can bold/italic the lede.
 * - "By"
 *   prefix:  UniformText, so the prefix can be localized per locale.
 * - Category: rendered as a `Badge` styled from a small hard-coded color
 *             map (technology, design, business, marketing, development,
 *             lifestyle). Unknown categories fall back to gray — add new
 *             keys to `categoryColors` to extend.
 * - Date:     formatted with `Intl` to `Month DD, YYYY` in `en-US`.
 *             Swap the locale if you need i18n-aware dates.
 * - Image:    `next/image fill` inside a 2:1 frame, transformed to
 *             1200×600 (`cover`, focal-point aware). Loaded with
 *             `priority` because it's above the fold.
 *
 * When no featured image is selected, a dashed-border placeholder with
 * a hint arrow is rendered in the editor so the layout stays stable.
 */
