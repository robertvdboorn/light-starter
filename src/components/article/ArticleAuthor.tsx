 ...
 *
 * Author bio block for the bottom of article pages. Renders a horizontal
 * layout: circular avatar on the left, name/bio/social links on the right,
 * separated from content above by a top border.
 *
 * Composition:
 * - Section heading: UniformText (`sectionTitle`), defaults to "About the Author".
 * - Avatar: Uniform asset → `imageFrom` at 160×160 (`cover`, focal-point aware),
 *   displayed in an 80×80 `rounded-full` via `next/image fill`.
 * - Name: UniformText (`name`).
 * - Bio: UniformRichText (`bio`) with the shared `resolveRichTextRenderer`.
 * - Social: `websiteUrl`, `twitterUrl`, `linkedinUrl` string parameters.
 *   Each icon link renders only when its URL is set; all open in a new tab
