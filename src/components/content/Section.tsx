 ...
 * Section Component
 *
 * General-purpose content block with six visual variants controlled by a
 * single `variant` select parameter — one component instead of six.
 *
 * Variants and their semantic role:
 * - `hero`        → `<h1>`, large type, no background (top of page)
 * - `content`     → `<h2>`, standard prose width (default)
 * - `feature`     → `<h2>`, blue tinted card (highlight a capability)
 * - `testimonial` → `<h3>`, green tinted card, italic body (quotes)
 * - `callout`     → `<h3>`, yellow tinted card (warnings / announcements)
 * - `minimal`     → `<h4>`, compact, no background (subtle content)
 *
 * Composition:
 * - Title:       UniformText, tag chosen per variant.
 * - Description: UniformRichText with `resolveRichTextRenderer`.
 * - CTA:         UniformSlot `cta` — accepts up to 3 `cta` components;
 *                hidden via `[&:empty]:hidden` when nothing is dropped in.
 */
